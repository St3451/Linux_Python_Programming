...
diff -y --suppress-common-lines british-english american-english | cut -d " " -f 1 | wc -w

# 5. Repeat the previous question, but now use the time command to measure its performance.
time diff -y --suppress-common-lines british-english american-english | cut -d " " -f 1 | wc -w
